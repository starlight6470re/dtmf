...
# 12/8/2003
# Jiwon Hahn
# Modified by erga0131 2025/02/25
...
def encoder(symbol):
    ...
    for i in range(16):
        if symbol == keys[i]:
            f1 = F1[i // 4] #row
            f2 = F2[i % 4] #column
            break #キー発見後、不要なループ処理を避ける
    data = []
    for i in range(FR):
        p = i*1.0/FR
        sample = int(scale * (sin(p * f1 * PI2) + sin(p * f2 * PI2)) / 2) #scaleの加算は不要らしいです
        data.append(sample)
    store_wav(data)
...
def store_wav(data):
    with wave.open('p2.wav', 'wb') as fout:
        ...
        #       8bitずつにわけ、bytesにrangeとして渡して変換する
            fout.writeframes(bytes([LS8bit, MS8bit]))

def read_wav():
    with wave.open('p2.wav','rb') as fin:
        n = fin.getnframes()
        d = fin.readframes(n)
    ...
    for f1 in F1:
        for f2 in F2:
            ...
            for i in range(FR): #assume phase has not shifted dramatically
                p = i*1.0/FR
                S = int(scale * (sin(p * f1 * PI2) + sin(p * f2 * PI2)) / 2) #scaleの加算は不要らしいです
                diff += abs(S-data[i])
...
def menu():
    while True:
        ...
        print ('*\t0\t#\tD')
        print ('**************************')
        X = input('Enter a key, or x to exit: ')
        if X not in keys:
            if X == 'x':
                exit(0)
            print ('Invalid key...')
